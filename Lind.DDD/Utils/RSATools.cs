...
namespace Lind.DDD.Utils
{
    ...
    public sealed class RSATools
    {
        ...
        #region 签名(privateKey)和验签(publicKey)
        /// <summary>
        /// 签名
        /// </summary>
        ...
        #region 加密(publicKey)和解密(privateKey)
        /// <summary>
        /// 用RSA解密
        /// </summary>
        ...
        public static string DecryptData(string resData, string privateKey, string input_charset = "utf-8")
        {
        ...
        /// <returns></returns>
        public static string EncryptData(byte[] data, string publicKey, string input_charset = "utf-8")
        {
        ...
        /// <summary>
        /// 将一级参数做ecode处理,并进行大写转换
        /// Author:Lind.zhang
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        public static string EncodeEx(string data)
        {
            ...
            for (int i = 0; i < temp.Length; i++)
            {
                ...
                var kv = new String[2];
                kv[0] = temp[i].Substring(0, temp[i].IndexOf('='));
                kv[1] = temp[i].Substring(temp[i].IndexOf('=') + 1); //避免有多个等于号的问题

                Console.WriteLine("kv{0},{1}", kv[0], kv[1]);
                result += kv[0] + "="
                                + Regex.Replace(Regex.Replace(Regex.Replace(Regex.Replace(Regex.Replace(Regex.Replace(Regex.Replace(Regex.Replace(System.Web.HttpUtility.UrlEncode(kv[1].ToString()), @"\+", "%20"), HttpUtility.UrlEncode("\""), HttpUtility.UrlEncode("\"").ToUpper()), HttpUtility.UrlEncode(":"), HttpUtility.UrlEncode(":").ToUpper()), HttpUtility.UrlEncode("{"), HttpUtility.UrlEncode("{").ToUpper()), HttpUtility.UrlEncode("}"), HttpUtility.UrlEncode("}").ToUpper()), HttpUtility.UrlEncode("&"), HttpUtility.UrlEncode("&").ToUpper()), HttpUtility.UrlEncode(","), HttpUtility.UrlEncode(",").ToUpper()), HttpUtility.UrlEncode("="), HttpUtility.UrlEncode("=").ToUpper())
                                + "&";
            }
