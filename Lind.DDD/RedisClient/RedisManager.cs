    ...
    public class RedisManager
    {
        ...
        static ConnectionMultiplexer GetCurrentRedis()
        {
            ...
            if (issentinel == 1)
            {
                ...
                {
                    Lind.DDD.Logger.LoggerFactory.Instance.Logger_Debug("join redis sentinel host:" + i);
                ...
                for (int i = 0; i < option.EndPoints.Count; i++)
                {
                    ...
                    {
                        Lind.DDD.Logger.LoggerFactory.Instance.Logger_Debug("GetCurrentRedis().RedisConnectionException" + ex.Message);
                        continue;
                    }
                    catch (Exception ex)
                    {
                        Lind.DDD.Logger.LoggerFactory.Instance.Logger_Debug("GetCurrentRedis().Exception" + ex.Message);
                        continue;
                    }
                }
